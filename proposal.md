...
[beRi](https://github.com/datasnakes/beRi) is a young project, which is being developed as a toolset for managing R installations, R packages, and R environments. Following the [UNIX philosophy](https://en.wikipedia.org/wiki/Unix_philosophy), we plan to divide these three goals into three modular packages, namely rinse, rut, and renv, respectively. These packages will form the basis for beRi, which will leverage their functionality and provide a unified and user-friendly experience for configuring system environments for R. Conda on the other hand is a well-developed and highly popular data science framework for package and environment management. While it is similar to beRi, there are some key differences that set conda and beRi apart for managing the R language. beRi’s primary goal is to provide a practical, intuitive, and convenient command-line interface for installing packages from CRAN, Bioconductor and other popular repositories such as MRAN, GitHub, and OmegaHat. [CRAN](https://cran.rstudio.com/web/packages/index.html) alone has a package count of 13,285 with around 6,600 packages being actively maintained or updated within the past year. At the time of writing, Anaconda [lists](https://docs.anaconda.com/anaconda/packages/r-language-pkg-docs/) 328 R packages available on Anaconda Cloud, which is conda's proprietary package management system. Bioconda is a third party repository for bioinformatics recipes for the conda frameowrk. While there are over 3000 Bioconductor recipes for the Ananconda system, there are only about 1,500 packages available on Bioconductor. The main takeaway here is that conda is a useful resource for anyone who performs analyses with R. However, beRi can serve as an equally useful alternative in cases where direct access to CRAN, Bioconductor, or other parts of the R ecosystem is needed.
